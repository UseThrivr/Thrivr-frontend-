...
const SalesCard: React.FC = () => {
    return (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 w-full mt-8 mb-[64px]">
            {salesData.map((item, index) => (
                <div
                    key={index}
                    className={`p-[16px] h-[134px] rounded-[8px] flex flex-col text-white ${item.color} gap-[48px]`}
                >
                    ...
                    <div className="flex justify-between items-center w-full h-[30px]">
                        <p className="text-base lg:text-xl leading-[30px] text-primary font-[500]">{item.amount}</p>
